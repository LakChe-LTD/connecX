...
import { createHotspot, type CreateHotspotData } from "@/api/services/hotspotService";
...
export default function CreateHotspotModal({ isOpen, onClose, onSuccess }: CreateHotspotModalProps) {
  const { theme } = useApp();
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      const response = await apiClient.post(ENDPOINTS.HOTSPOT.LIST, formData);

      if (response.data.success) {
        setSuccess(true);
        
        // Call success callback with the created hotspot
      ...
      const errorMessage = err.response?.data?.message || err.message || "Failed to create hotspot";
        ...
        {success && (
          <div className="absolute inset-0 z-10 flex items-center justify-center bg-black bg-opacity-80 rounded-xl">
            <div className="text-center">
              ...
              <h3 className="text-2xl font-bold text-white mb-2">Hotspot Created!</h3>
              <p className="text-gray-300">Your hotspot has been successfully created.</p>
            </div>
          </div>
        )}
